<template>
  <div class="musics">
    <div class="title">
      ...
      <div class="big_tit">音乐</div>
      <div><i class="iconfont icon-20"></i></div>
    </div>
    <div class="feed_cover" :style="{background:'url('+this.music.feeds_cover+')'}">
      <div class="cover">
        <img :src="this.music.cover" width="100%">
        <p>{{this.music.title}}</p>
      </div>
    </div>
    <div class="content">
      <div class="cont_title">{{this.music.story_title}}</div>
      <div class="cont_author">文 / {{this.author.user_name}}</div>
      <div class="cont_content" v-html="this.music.story"></div>
      <div class="duty">{{this.music.charge_edt}}</div>
      <div class="author">
    ...
    <div class="bottom">
      ...
      <div class="bottom_icon">
        <div><i class="iconfont icon-shoucang"></i>{{this.music.praisenum}}</div>
        <div><i class="iconfont icon-pinglun"></i>{{this.music.sharenum}}</div>
        <div><i class="iconfont icon-zhuanfa"></i>{{this.music.commentnum}}</div>
      </div>
...
export default {
  ...
  computed:{
    ...mapGetters('music',['music','author'])
  },
  methods:{
    ...mapActions('music',['getMusicById']),
    goBack(){
      this.$router.go(-1);;
      showNav();
    }
...
.cover{
  ...
  p{
    height: remOne(126px);
    color: #d9d9d9;
    line-height:  remOne(126px);
    font-size:remOne(18px)
  }
}
.musics{
