<template>
  <div class="serial">
    <div class="title">
      ...
      <div class="big_tit">连载</div>
      <div><i class="iconfont icon-20"></i></div>
    </div>
    <div class="content">
      <div class="cont_title">{{this.serial.title}}</div>
      <div class="cont_author">文 / {{this.author.user_name}}</div>
      <div class="cont_content" v-html="this.serial.content"></div>
      <div class="duty">{{this.serial.charge_edt}}</div>
      <div class="author">
    ...
    <div class="bottom">
      ...
      <div class="bottom_icon">
        <div><i class="iconfont icon-shoucang"></i>{{this.serial.praisenum}}</div>
        <div><i class="iconfont icon-pinglun"></i>{{this.serial.sharenum}}</div>
        <div><i class="iconfont icon-zhuanfa"></i>{{this.serial.commentnum}}</div>
      </div>
...
export default {
  ...
  created(){
    this.getSerial(this.$route.params.id)
  },
  computed:{
    ...mapGetters('read',['serial','author'])
  },
  methods:{
    ...mapActions('read',['getSerial']),
    goBack(){
      this.$router.go(-1);;
      showNav();
    }
